...
TARGETS = ['rivera', 'mesa boogie', 'transatlantic', 'stratocaster', 'fender' 'les paul', 'carvin' ]
...
rss_content = RSS::Maker.make(RSS_VERSION) do |_rss|
  ...
  _rss.channel.description = 'Musical instruments found after searching for: %s' % (ARGV | TARGETS).join(', ')
  ...
  (ARGV.map{ |_i| _i.downcase } + TARGETS).each do |_target|
    ...
    page.css('p.row').each do |_result|
      ...
      item.date  = Time.parse(desc[0 .. 5]) rescue Time.now
    end
  end
end
